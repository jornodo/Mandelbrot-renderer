...
# ╔═╡ 31fa3a50-15e5-11ed-29a4-c7a81a00e0b9
begin
	using CUDA
	using Images
	using Colors
...
# ╔═╡ a732984a-fe76-46fc-a9a3-8dfe7497de43
# function for sequentially calculating the mandelbrot running on cpu
# resolution will be the same for x and y axis
seqMandelbrot(x, y, range, iterations, resolution) = begin
	calculatePoint(c) = begin
		n = 0
		z = 0
		...
		# For generating color later, we are interested in two things:
		# 1: The amount of iterations needed; wether the point converges or not
		
		# 2: The value of z when the loop terminates. This will tell us something about how fast the point converges towards infinity.
		
		# returns 0 for points in the set as to color those black
	...
	stepSize = abs(-range - range) / resolution

	# returning a matrix of tuples from calculating each points value in the mandelbrot set.
	[calculatePoint(complex(i, j)) for i in minX:stepSize:maxX, j in minY:stepSize:maxY]
end
...
makeFractalImage(seqMandelbrot(-.5, 0, 2.5, 1000, 750))

# ╔═╡ 4c791c79-f296-41f0-a582-f3b1d85465a0
# Implementing the calculations for cuda.
cudaMandelbrot(x, y, range, iterations, resolution) = begin
	
	# Again defining the calculatePoint function, with explicit typing for CUDA
	# This time we will take a tuple as the argument. 
	# The tuple will contain the coordinates of each point in the array.
	calculatePoint(tuple::Tuple{Float64, Float64})::Tuple{Float64, Float64} = begin
		c::Complex = complex(tuple[1], tuple[2])
		n = 0
		z = 0
	...
	# Making a CuArray filled with the coordinates for each point
...
# ╔═╡ 5eea3147-2120-4eca-9bb5-9c30179e4a5d
# x, y, range, iterations, resolution
makeFractalImage(Array(cudaMandelbrot(-.5, 0, 2.5, 1000, 750)))
...
begin
	...
	# Generating the image is done on the CPU, but could possibly be more efficient 
	# On GPU for high resolution images.

	# However, I'm not too worried about that right now; as iterating over the matrix once is generally quite fast.
end
...
# function for generating a series of images gradually zooming in on the set.
# zoomrate < 1 magnifies, >1 minimizes
makeZoom(zoomRate, range, x, y, amountOfRenders, resolution, iterations) = begin
	renderRange = range
	renderArray = Array{Array{RGB{Float64}, 2}}(undef, amountOfRenders)
	
	# ------------ arguments ------------
	# x, y, range, iterations, resolution
	for i in 1:amountOfRenders
		temp = cudaMandelbrot(x, y, renderRange, iterations, resolution)
		renderArray[i] = makeFractalImageCuda(temp)

		# Ensures that memory on gpu is freed in order to save time used on GC for large amounts of renders
		CUDA.unsafe_free!(temp)

		renderRange *= zoomRate
	end
...
# Takes Array of RGB Matrices and converts them into a 3D Matrix of RGB objects
# This is needed for saving as gif
